...
class ReadDeniedIfRoleOfCurrentUserIsAuthorOrObserverAndCurrentUserGroupNotEqualEntityValidatorTest {
    ...
    void checkAccessBeforeRead_shouldGetNoError_whenCurrentUserExecutorAdminOrAccountOwnerInnerGroup(String role, Boolean isInnerGroup) {
    ...
    @WithUserDetails("EXECUTOR_ACCOUNT_1_IS_NOT_INNER_GROUP")
    void checkAccessBeforeRead_shouldGetAccessDeniedException_whenUsersRoleObserverOrAuthor(String role, Boolean isInnerGroup) {
        var groupForRead = groupTestHelper.getRandomValidEntity();
        var currentUser = userTestHelper.getRandomValidEntity();
        currentUser.setRole(roleTestHelper.getRoleByName(role));
        currentUser.getGroup().setIsInner(isInnerGroup);
